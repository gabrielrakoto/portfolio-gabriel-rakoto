...
- Correction des points 🟠/🟡 (2026-09-03) :
  - Open Graph + Twitter Card (`summary_large_image`) ajoutés sur les 5 pages, avec une image de partage dédiée générée sur mesure (`assets/og-image.jpg`, 1200×630, rendue depuis un SVG via `sharp` en ligne de commande — pas d'outil de capture d'écran disponible, donc génération programmatique plutôt que design manuel).
  - JSON-LD `ProfessionalService` (schema.org) ajouté sur `index.html` uniquement (entité canonique du site) : nom, description, email, téléphone, zone desservie (Québec), fondateur, lien LinkedIn — pas d'adresse postale publique puisque Gabriel est travailleur autonome sans local commercial (service-area business).
  - `robots.txt` (`Allow: /`, référence au sitemap) et `sitemap.xml` (5 URLs, priorités par importance) créés à la racine.
  - `alt` descriptifs générés dynamiquement dans `js/main.js` pour les images de projets (`"Capture d'écran du site web réalisé pour [titre] — [niche]"`), remplaçant les `alt=""` vides.
  - Images de projets utilisées (`kami-auto-garage`, `lotus-imperial`, `paysagement-pro`, `plomberie-pro`) converties en WebP (`sharp`, qualité 80, ~30 % plus légères) et servies via `<picture>` + `<source type="image/webp">` avec fallback JPEG (le `<picture>` a `style="display:contents"` pour ne pas casser le layout CSS existant qui ciblait directement `<img>`).
  - Images orphelines jamais utilisées (`delice-de-manille.png`, `jmc-mecanique.png`, ~2 Mo à elles deux) supprimées du repo après vérification qu'aucune référence n'existait dans le code.
